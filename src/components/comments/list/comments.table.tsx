...
const CommentsTable = (props: { page: string, postUuid: string, pageSize?: number, onNavigate?: (page: number) => void }) => {
  ...
  const fetchComments = async () => {
    ...
    try {
      const response = await FEED_SERVICE.getComments({ offset: parseInt(props.page) * MAX_COMMENTS_PER_PAGE, limit: LIMIT, postUuid: props.postUuid })
      clearTimeout(timer)
  ...
  React.useEffect(() => {
    fetchComments()
  }, [props.page, props.postUuid, props.pageSize])
      ...
      <table className="table-auto flex-1">
        ...
        <tbody>
          {comments.map(function (p: FeedComment, idx) {
            return (
              <CommentRow key={idx} comment={p} postUuid={props.postUuid} />
            )
          })}
        </tbody>
      </table>
